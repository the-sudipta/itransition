...
if(isset($_SESSION["user_id"]) && isset($_SESSION["user_role"])){
    ...
    if(!isset($_SESSION['role_error'])){
        $_SESSION['role_error'] = false;
    }
    if($_SESSION["user_id"] > 0){
        // Valid User Logged in
        // Is he trying to access other roles pages?
        if($_SESSION['role_error'] === true){
            // Check the role first
            if(strtolower($_SESSION["user_role"]) === "user"){
                $route = $admin_dashboard_page;
                $error_message = "Direct URL access to salesman roles pages is not allowed. Use the proper interface";
            }else{
                $route = $salesman_dashboard_page;
                $error_message = "Direct URL access to admin roles pages is not allowed. Use the proper interface";
            }
        }elseif (isset($_SESSION['backend_direct_access'])){
            if($_SESSION['backend_direct_access'] === true){
                if(strtolower($_SESSION["user_role"]) === "admin"){
                    $route = $admin_dashboard_page;
                    $error_message = "Direct URL access to backend is not allowed. Use the proper interface";
                }else{
                    $route = $salesman_dashboard_page;
                    $error_message = "Direct URL access to backend is not allowed. Use the proper interface";
                }
...
<main>
  ...
  <div class="flexy-center">
    ...
    <div class="container__title" id="container__title"
      ...
      <div class="button__container">
          <a href="<?php echo $route.'?'.$error_type.'='.$error_message;?>" class="custom-button" id="return">
              R E T U R N <img src="<?php echo $return_icon_green; ?>" alt="Return Icon" class="icon">
          </a>
...
<!-- partial -->
  <script src='<?php echo $jquery_min_script; ?>'></script>
  <script>
